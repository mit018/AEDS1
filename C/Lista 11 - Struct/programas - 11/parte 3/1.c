...
3 médicos e no máximo 30 consultas (2 consultas por médico por dia) e salva esses dados*/
...
int c = 0; // número de consultas criadas
...
int cadastroConsulta(int i)
{
    ...
    while (contMedicoDia[x][consulta[i].dia].cont == 2)
    {
        ...
        if (op == 0)
        {
            ...
            c--;                  // diminui o número de consultas
            return 0;
        }
        else if (op == 1)
        {
...
void salvar()
{
    ...
    while (paciente[i].codigo != 0)
    {
        ...
        fprintf(arq, "\n\nPaciente %i\nCódigo: %i\nNome: %s\nEndereço: %s\nTelefone: %i", i + 1, paciente[i].codigo, paciente[i].nome, paciente[i].endereco, paciente[i].telefone);
        i++;
        fclose(arq);
    }
    i = 0;
    while (medico[i].codigo != 0)
    {
        cont = 1;
        arq = fopen("dados.txt", "a");
        if (!arq)
        ...
        fprintf(arq, "\n\nMédico(a) %i\nCódigo: %i\nNome: %s\nTelefone: %i", i + 1, medico[i].codigo, medico[i].nome, medico[i].telefone);
        i++;
        fclose(arq);
    }
    for (i = 0; i < c; i++)
    {
        if (consulta[i].num != 0)
        {
            ...
            fprintf(arq, "\n\nConsulta %i\nNúmero: %i\nDia da semana: %i° feira\nHorário: %s\nCódigo do(a) médico(a): %i\nCódigo do paciente: %i", i + 1, consulta[i].num, consulta[i].dia, consulta[i].hora, consulta[i].codMedico, consulta[i].codPaciente);
            fclose(arq);
        }
    }
...
int main()
{
    ...
    while (op != 0)
    {
        ...
        else if (op == 3)
        {
            if (c > 30)
            {
                printf("Cadastro de novas consultas indisponível\nMotivo: limite atingido (apenas 2 consultas por medico)\n");
            }
            else
            {
